...
```¬ LanguageOf zimbabwe dutch = !!NOT!! v(LanguageOf zimbabwe dutch) = !!NOT!! 0 = 1```
...
just like before, just that `v(`A`)∈{ 0, 1, ω }` rather than just `{0,
1}`. 
...
Here are, for example, the propositions `⟦¬SUN⟧` and `⟦SUN ∨RAIN⟧`:
...
```P₁, P₂, … ⊨ C &emsp; if and only if &emsp; ⟦P₁⟧∩⟦ P₂⟧∩ … ⊆ ⟦C⟧```
...
The shaded area is the intersection of `⟦¬SUN⟧∩⟦SUN ∨RAIN⟧` of
`⟦¬SUN⟧` and `⟦SUN ∨RAIN⟧`. And
...
`⟦RAIN⟧`. So, the inference is valid: from the
...
interpret the conditional ```A → B``` in the same way as in classical
...
formula of the form `A ∨¬A`. But, as it turns out, there are
...
Take the following formula, for example: ```RAIN∨¬RAIN```
...
```v(RAIN ∨¬RAIN) = ω ~!OR!~ (~!NOT!~ ω) = ω ~!OR!~ ω = ω```

But `ω ≠ 1`, which means that in any such model, where `v(RAIN) = ω`, is a model
where `RAIN∨¬RAIN` doesn't have value `1`. Under the
...
like `RAIN∨¬RAIN` are tautologies, and which fail in `K3`.
...
satisfy `{ P₁, P₂, …, ¬ C }`. A `K3` countermodel is a model where
...
The model which shows that `RAIN∨¬RAIN` doesn't follow from
`SUN` in `K3`, is a case in point: it's a model where `RAIN`, and thus 
`RAIN∨¬RAIN` is `ω`. But crucially, the negation of 
`RAIN∨¬RAIN`, that is 
`¬(RAIN∨¬RAIN)`, is unsatisfiable—and so is every set that contains it, like 
```{ SUN, ¬(RAIN∨¬RAIN) }```
For to make  `¬(RAIN∨¬RAIN)` have value `1`, the formula 
`RAIN∨¬RAIN` would need to have value `0`. But the only way
that could happen, even in `K3`, is if both `RAIN` and `¬RAIN` have
...
formula `RAIN∨¬RAIN` where we let ``v(A)` = X`. In other
words, the query searches for a countermodel to the formula `RAIN∨¬RAIN`. It does find one, and so we can conclude that the formula is not a
`K3` tautology.
...
`¬RAINBOW`, this would be a very good reason to assign `RAINBOW` the
...
model: we have a (non-empty) domain, `D`, and denotations `⟦a⟧∈D` for all the constants. The main difference to an FOL
...
with each predicate `Pⁿ` a function `⟦Pⁿ⟧`,
...
```⟦IsHot t⟧ = ⟦IsHot ⟧ ⟦t⟧```
Here, `⟦t⟧` is the denotation of the term `t`,
...
example,  `⟦ IsCold  10°C⟧` entails, in a sense,
`⟦ IsCold  10°C ∨ IsWarm 10°C⟧` since
...
`⟦P₁⟧, ⟦P₂⟧, …`
is smaller than or equal to the value `⟦C⟧`,
that is:

```P₁, P₂, … ⊨ C &emsp; if and only if, in all models,  min {⟦P₁⟧, ⟦P₂⟧, …} ≤ ⟦C⟧```
