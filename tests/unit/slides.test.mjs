...
for (const n of [1, 2, 3, 4, 5, 6]) test(`released Lecture ${n} has a current complete export`, async () => {
...
test('unreviewed slide originals and exports cannot enter the Git release', async () => {
  ...
  const released = ['slides/sources/Lecture 1.excalidraw', 'slides/sources/Lecture 2.excalidraw', 'slides/sources/Lecture 3.excalidraw', 'slides/sources/Lecture 4.excalidraw', 'slides/sources/Lecture 5.excalidraw', 'slides/sources/Lecture 6.excalidraw'];
  const releasedDecks = ['content/slides/logic-and-ai/deck/', 'content/slides/formal-languages/deck/', 'content/slides/valid-inference/deck/', 'content/slides/boolean/deck/', 'content/slides/sat/deck/', 'content/slides/conditionals/deck/'];
  ...
  for (const file of ['slides/images/example.png', 'slides/archive/Lecture 1.excalidraw.gz', 'content/slides/proof/deck/slide-01.svg']) {
    assert.equal(execFileSync('git', ['check-ignore', '--no-index', file], { encoding: 'utf8' }).trim(), file);
  }
});
